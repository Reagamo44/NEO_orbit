...
def initial_elliptical_state(alt_p: float = 500e3, alt_a: float = 2000e3, R: float = R_Earth, mu: float = MU_Earth    ) -> np.ndarray:
    ...
    a = (rp + ra)/2.0  # [m] semi-major axis

    vp = np.sqrt(mu * (2.0/rp - 1.0/a))  # [m/s] velocity at periapsis
    state0 = np.array([rp, 0, 0, 0, vp, 0], dtype=float) # initial state: position (x, y, z) and velocity (vx, vy, vz)

    return state0, mu, ra, rp, a # starting at (r_norm, 0, 0) with velocity (0, v_circular, 0)

def test_elliptical_orbit(plot: bool = True):

    # Parameters for a circular orbit
    state0, mu, ra, rp, a = initial_elliptical_state()
    ...
    r_min =  np.inf # [m] initialize minimum orbital radius to a very large value
    ...
    assert max_h_dev/h0_norm < 1e-9, f"Angular momentum deviated from initial value: {angular_momentum[-1]} m^2/s" # allow for small numerical deviations in angular momentum
    assert max_eps_dev/abs(eps0) < 1e-9, f"Energy deviated from initial value: {energy[-1]} J/kg" # allow for small numerical deviations in energy
